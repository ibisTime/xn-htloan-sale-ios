...
-(void)loadData
{
    ...
    for (int j = 0; j < self.model.creditUserList.count; j ++) {
        ...
        if ([self.model.creditUserList[j][@"loanRole"] isEqualToString:@"1"]) {
            ...
            idCardInfo = @{@"userName":[BaseModel convertNull:creditUser[@"userName"]],
                           @"nation":[BaseModel convertNull:creditUser[@"nation"]],
                           @"gender":[BaseModel convertNull:creditUser[@"gender"]],
                           @"customerBirth":[BaseModel convertNull:creditUser[@"customerBirth"]],
                           @"idNo":[BaseModel convertNull:creditUser[@"idNo"]],
                           @"birthAddress":[BaseModel convertNull:creditUser[@"birthAddress"]],
                           @"authref":[BaseModel convertNull:creditUser[@"authref"]],
                           @"statdate":[BaseModel convertNull:creditUser[@"statdate"]]
                           };
    ...
    if (self.model) {
        ...
        modelId = self.model.carInfoRes[@"region"];
    ...
    [http postWithSuccess:^(id responseObject) {
        ...
        view.selectedBlock = ^(NSArray<SelectedListModel *> *array) {
            [LEEAlert closeWithCompletionBlock:^{
                ...
                zone = _brandAry[model.sid][@"cityId"];
                [self getreport];
            }];
                ...
                [http postWithSuccess:^(id responseObject) {
                    ...
                    view.selectedBlock = ^(NSArray<SelectedListModel *> *array) {
                        [LEEAlert closeWithCompletionBlock:^{
                            ...
                            http.showView = self.view;
                            http.code = @"630426";
                            http.parameters[@"seriesCode"] = _carsDic[@"code"];
                            http.parameters[@"status"] = @"1";
                            http.parameters[@"type"] = @"1";
                            [http postWithSuccess:^(id responseObject) {
