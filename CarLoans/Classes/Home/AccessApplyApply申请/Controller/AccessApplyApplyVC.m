...
-(void)TheReturnValueStr:(NSString *)Str selectDic:(NSDictionary *)dic selectSid:(NSInteger)sid
{
    switch (selectCell) {
        ...
        case 1:
        {
            ...
            NSIndexPath *ReloadIndexPath1 = [NSIndexPath indexPathForRow:0 inSection:11]; 
...
-(void)initScrollView
{
    ...
    _scroll.contentSize = CGSizeMake(SCREEN_WIDTH * 10, 0);
...
-(void)navigativeView
{
    UIView *naviView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kNavigationBarHeight)];
    naviView.backgroundColor = kAppCustomMainColor;
    [self.view addSubview:naviView];
    self.LeftBackbButton.titleLabel.font = HGfont(16);
    self.LeftBackbButton.frame = CGRectMake(15, kStatusBarHeight, 100, 44);
    self.RightButton.titleLabel.font = HGfont(16);
    self.RightButton.frame = CGRectMake(SCREEN_WIDTH - 115, kStatusBarHeight, 100, 44);
    [naviView addSubview:self.RightButton];
    [naviView addSubview:self.LeftBackbButton];
    ...
    _titleBtn = [UIButton buttonWithTitle:@"" titleColor:[UIColor whiteColor] backgroundColor:kClearColor titleFont:16];
    _titleBtn.frame = CGRectMake(SCREEN_WIDTH/2 - 75, kStatusBarHeight, 150, 44);
    [_titleBtn addTarget:self action:@selector(titleBtnClick) forControlEvents:(UIControlEventTouchUpInside)];
    [naviView addSubview:_titleBtn];
    [self titleBtnCustom];
}
