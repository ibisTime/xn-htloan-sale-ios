...
@interface ToApplyForRightTableView6 ()<UITableViewDataSource,UITableViewDelegate>

@end
@implementation ToApplyForRightTableView6
...
//        NSArray *topArray = @[@"*姓名",@"*与主贷人关系",@"*手机号",@"*身份证号",@"*学历",@"*户籍地（省市区）",@"*户籍地（详细地址）",@"*户籍地邮编",@"*工作单位名称",@"*工作单位地址",@"*工作单位电话"];
        if (indexPath.row == 0 || indexPath.row == 1 || indexPath.row == 2 || indexPath.row == 3 ) {
            cell.type = ShowType;
            cell.showLbl.tag = 60000 + indexPath.row;
        }else if (indexPath.row == 5)
        {
            cell.type = ChooseType;
            cell.chooseLbl.tag = 60000 + indexPath.row;
        }
        else
        {
            cell.type = InputType;
